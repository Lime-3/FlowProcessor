"""Test suite for the visualize_metric function in flowproc.gui.visualizer.
...
from flowproc.gui.visualizer import visualize_metric
import hashlib
import logging
...
def mock_visualization_env():
    ...
    with patch("flowproc.visualize.visualize_data") as mock_viz:
        mock_viz.return_value = None  # Simulate successful visualization
        with patch("flowproc.gui.visualizer.QDialog.exec") as mock_exec:
            with patch("flowproc.gui.visualizer.QWebEngineView.load") as mock_load:
...
def test_visualize_metric_success(qtbot, mock_visualization_env):
    ...
    with patch("flowproc.visualize.load_and_parse_df") as mock_parse:
