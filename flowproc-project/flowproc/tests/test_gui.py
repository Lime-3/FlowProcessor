...
def test_visualize_selected_permission_denied(qapp, temp_csv, mock_home, monkeypatch):
    ...
    monkeypatch.setattr("flowproc.gui.main.visualize_data", mock_visualize)

    mock_window = MagicMock()
    mock_msgbox = MagicMock()
    monkeypatch.setattr("flowproc.gui.main.window", mock_window)
    monkeypatch.setattr("PySide6.QtWidgets.QMessageBox.critical", mock_msgbox)
    monkeypatch.setattr("flowproc.gui.main.visualize_combo", MagicMock(currentText=MagicMock(return_value="Freq. of Parent")))
    monkeypatch.setattr("flowproc.gui.main.time_course_checkbox", MagicMock(isChecked=MagicMock(return_value=False)))
    monkeypatch.setattr("flowproc.gui.main.USER_REPLICATES", [1, 2, 3])
    monkeypatch.setattr("flowproc.gui.main.AUTO_PARSE_GROUPS", True)
    monkeypatch.setattr("flowproc.gui.main.USER_GROUP_LABELS", [])
