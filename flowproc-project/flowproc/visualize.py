...
from flowproc.plotting import apply_plot_style, PLOT_STYLE
...
class VisualizationConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if self.metric and self.metric not in KEYWORDS:
            raise ValueError(f"Invalid metric '{self.metric}'. Valid options: {list(KEYWORDS.keys())}")
        ...
        if self.config.metric:
            ...
            if not metrics:
                raise DataProcessingError(
                    f"Invalid metric '{self.config.metric}'. Valid options: {list(KEYWORDS.keys())}"
                )
            return metrics
